...
public class DynamicProxyFactory {
	...
	static Map<Class<?>,String> prim_wrapper_table = new HashMap<Class<?>,String>(8);
	static {
	...
	static String wrapperClass(Class<?> returnType) {
		return prim_wrapper_table.get(returnType);
	}
	...
	static void castParam(ClassFile cf, Class<?> paramType) {
		if (paramType == int.class) {
			cf.add(Opcode.INVOKESTATIC, "dynamicinvoke.impl.DynamicProxyFactory",
					"int_cast", "(Ljava/lang/Object;)", "I");
		} else if (paramType == byte.class) {
			cf.add(Opcode.INVOKESTATIC, "dynamicinvoke.impl.DynamicProxyFactory",
					"byte_cast", "(Ljava/lang/Object;)", "B");
		} else if (paramType == short.class) {
			cf.add(Opcode.INVOKESTATIC, "dynamicinvoke.impl.DynamicProxyFactory",
					"short_cast", "(Ljava/lang/Object;)", "S");
		} else if (paramType == char.class) {
			cf.add(Opcode.INVOKESTATIC, "dynamicinvoke.impl.DynamicProxyFactory",
					"char_cast", "(Ljava/lang/Object;)", "C");
		} else if (paramType == long.class) {
			cf.add(Opcode.INVOKESTATIC, "dynamicinvoke.impl.DynamicProxyFactory",
					"long_cast", "(Ljava/lang/Object;)", "J");
		} else if (paramType == float.class) {
			cf.add(Opcode.INVOKESTATIC, "dynamicinvoke.impl.DynamicProxyFactory",
					"float_cast", "(Ljava/lang/Object;)", "F");
		} else if (paramType == double.class) {
			cf.add(Opcode.INVOKESTATIC, "dynamicinvoke.impl.DynamicProxyFactory",
					"double_cast", "(Ljava/lang/Object;)", "D");
	...
	public static DynamicProxy makeProxy(Method method, CodeLoader loader) {
		return makeProxy(method.getName(), method.getDeclaringClass(),
				method.getReturnType(), method.getParameterTypes(),
				Modifier.isStatic(method.getModifiers()) ? 1 : 0, loader);
	}

	public static DynamicProxy makeProxy(String methodName, Class<?> declaringClass,
			Class<?> returnType, Class[] paramTypes, int type, // 0: instance
															// method, 1: static
															// method, 2:
															// constructor
			CodeLoader loader) {
		...
		if (type == 1) {
			cf.add(Opcode.INVOKESTATIC, className, methodName,
					ClassFile.signature(paramTypes),
					ClassFile.signature(returnType));
		} else if (type == 0) {
			if (declaringClass.isInterface()) {
				cf.add(Opcode.INVOKEINTERFACE, className, methodName,
						ClassFile.signature(paramTypes),
						ClassFile.signature(returnType));
			} else {
				cf.add(Opcode.INVOKEVIRTUAL, className, methodName,
						ClassFile.signature(paramTypes),
						ClassFile.signature(returnType));
			}
		} else if (type == 2) {
			cf.add(Opcode.INVOKESPECIAL, className, methodName,
					ClassFile.signature(paramTypes),
					ClassFile.signature(returnType));
		}

		if (prim) {
			cf.add(Opcode.INVOKESPECIAL, wrapperClass(returnType), "<init>",
					"(" + ClassFile.signature(returnType) + ")", "V");
		}
	...
	/* The following methods are called by generated code */
